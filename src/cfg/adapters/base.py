...
class StorageAdapter(Protocol):
    ...
    # OPTIONAL cheap ref list for Engine._all_refs. Returns just the distinct
    # (collection, record_id) pairs that have history — no doc, no per-row object
    # materialization. Callers feature-detect and fall back to query_history when
    # absent. On a remote store this is the difference between ~160ms and ~5s per load.
